...
if [[ "${REMOTE_COLLECT}" == "1" ]]; then
  ssh -x "${REMOTE_HOST}" "cd ${REMOTE_DIR}/../.. && eval \"\$(scramv1 runtime -sh)\" && cd ${REMOTE_DIR} && DASHBOARD_ENSURE_PROXY='${DASHBOARD_ENSURE_PROXY:-0}' DASHBOARD_PROXY_MIN_VALID='${DASHBOARD_PROXY_MIN_VALID:-4:00}' DASHBOARD_PROXY_VALID='${DASHBOARD_PROXY_VALID:-192:00}' DASHBOARD_CONDOR_HISTORY_LIMIT='${DASHBOARD_CONDOR_HISTORY_LIMIT:-200}' DASHBOARD_CONDOR_SCHEDDS='${DASHBOARD_CONDOR_SCHEDDS:-}' DASHBOARD_CRAB_TASK_GLOBS='${DASHBOARD_CRAB_TASK_GLOBS:-}' DASHBOARD_CRAB_WORKERS='${DASHBOARD_CRAB_WORKERS:-8}' DASHBOARD_CRAB_TIMEOUT_SEC='${DASHBOARD_CRAB_TIMEOUT_SEC:-120}' DASHBOARD_OUTPUT_MAPPING_SCRIPTS='${DASHBOARD_OUTPUT_MAPPING_SCRIPTS:-}' DASHBOARD_DERIVED_OUTPUT_PREFIXES='${DASHBOARD_DERIVED_OUTPUT_PREFIXES:-JetMET=/store/group/lpcdisapptrks/ntuplizer}' DASHBOARD_EOS_ENDPOINT='${DASHBOARD_EOS_ENDPOINT:-root://cmseosmgm01.fnal.gov}' DASHBOARD_EOS_ROOTS='${DASHBOARD_EOS_ROOTS:-}' bash collectors/collect_all.sh"
fi

rsync -av -e "ssh -x" "${REMOTE_HOST}:${REMOTE_DIR%/}/snapshots/"*.json "${LOCAL_DIR}/snapshots/"

PYTHON_BIN="${LOCAL_DIR}/.venv/bin/python"
...
(
  cd "${LOCAL_DIR}"
  "${PYTHON_BIN}" -c "from dashboard.db import initialize_database; from dashboard.production_tasks import sync_production_tasks; initialize_database(); print(sync_production_tasks())"
)
